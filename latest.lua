...
Title.TextSize = 17
Title.Text = "Fatality yeld " .. currentVersion
...
addcmd('chamsred', {}, function(args, speaker)
    -- Функция для изменения материала и цвета
    local function changeMaterialToForceField(object)
        if object:IsA("BasePart") then
            object.Material = Enum.Material.ForceField
            object.Color = Color3.fromRGB(255, 0, 0) -- Красный цвет
        end
    end
    ...
    -- Применяем изменения к существующим потомкам
    for _, descendant in pairs(workspace.CurrentCamera:GetDescendants()) do
        changeMaterialToForceField(descendant)
    end
end)

addcmd('3ebalo', {}, function(args, speaker)
    local player = game.Players.LocalPlayer

    -- Функция для переключения в третий режим камеры
    ...
    -- Слушатель изменений свойства CameraMode
    player:GetPropertyChangedSignal("CameraMode"):Connect(function()
        ensureThirdPerson()
    end)

    -- Убедиться, что сразу переключаемся в третий режим
    ensureThirdPerson()
end)

addcmd('falldmgwt', {}, function(args, speaker)
    -- Проверяем и удаляем объект "Freefall" в ReplicatedStorage
    local freefall = game:GetService("ReplicatedStorage"):FindFirstChild("Freefall")
    if freefall then
        freefall:Destroy()
    else
        print("nuigga") -- Сообщение, если объект не найден
    end

    -- Проверяем и удаляем объект "FDMG" в ACS_Engine -> Events
...
addcmd('infammo', {}, function(args, speaker)
    _G.skurilpizdu = true
...
addcmd('rpgspam', {}, function(args, speaker)
    -- Начать серию выстрелов
    isFiring = true
    while isFiring do
        fireQuickLagRocket()
        wait(0.1)  -- Пауза между выстрелами (можно настроить по своему усмотрению)
    end
end)

-- Остановить серию выстрелов, если необходимо
addcmd('unrpgspam', {}, function(args, speaker)
    isFiring = false
end)
